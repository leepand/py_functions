    ...
    def write_api(self, api_name, group_data=None, data=None):
        ...
        if group_data:
            for item in group_data:
                self.doc.add_paragraph(style="Heading 1").add_run(
                    str(index) + "、" + item["name"]
                ).font.size = 300000
                _id = 1
                for items in item["First"]:
                    self.doc.add_paragraph(style="Heading 2").add_run(
                        str(_id) + "." + items["name"], style="Default Paragraph Font"
                    )
                    _id = _id + 1
                    self.doc.add_paragraph(
                        "简要描述：", style="Body Text"
                    ...
                    if items["httpType"] == "HTTP":
                        self.doc.add_paragraph(
                            "http://xxxx" + items["apiAddress"], style="List Paragraph"
                        )
                    else:
                        self.doc.add_paragraph(
                            "https://xxxx" + items["apiAddress"], style="List Paragraph"
                        )
                    ...
                    if items["requestParameterType"] == "form-data":
                        table = self.doc.add_table(
                            rows=len(items["requestParameter"]) + 1,
                            cols=4,
                        ...
                        for row in table.rows[1:]:
                            ...
                            if items["requestParameter"][j]["required"]:
                                ...
                                row.cells[2].text = "否"
                            try:
                                row.cells[3].text = items["requestParameter"][j][
                                    "restrict"
                                ]
                            except TypeError:
                                pass
                        ...
                        try:
                            if len(items["requestParameterRaw"]):
                                data = json.loads(
                                    items["requestParameterRaw"][0]["data"]
                                )
