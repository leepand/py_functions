...
df["genre"] = np.array(
    ["action", "romance", "action", "sf", "action", "horror", "sf", "horror", "action"]
)
print(df)
...
def convert_onehot_to_category(df, id_col, one_hot_columns, category_col="category"):
    ...
    # 重複削除
    df_concat = (
        df_concat.drop_duplicates().reset_index(drop=True).sort_values(by=id_col)
    )
    return df_concat
...
df_category = convert_onehot_to_category(
    df_onehot, id_col=id_col, one_hot_columns=genres, category_col=category_col
)
