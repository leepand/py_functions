...
inputs.append(
    tf.keras.preprocessing.sequence.pad_sequences(
        df_seq["categoryid"].values.tolist(),
        padding="post",
        truncating="post",
        maxlen=10,
    )
...
def create_list(df, user_index_col, sort_col, target_col, user_num):
    ...
    for _, user_index, sort_value, target_value in df[
        [user_index_col, sort_col, target_col]
    ].itertuples():
...
itemid_inputs = create_list(
    df, user_index_col="userid", sort_col="timestamp", target_col="itemid", user_num=3
)
categoryid_inputs = create_list(
    df,
    user_index_col="userid",
    sort_col="timestamp",
    target_col="categoryid",
    user_num=3,
)
...
def sort_list(inputs, is_descending):
    ...
    """
    return [
        sorted(i_input, key=lambda i: i[1], reverse=is_descending) for i_input in inputs
    ]
